...
if __name__ == "__main__":
    ...
    if args.onnx:
        model = OPTForCausalLM.from_pretrained("facebook/" + args.model_name, cache_dir="./cache")
        ...
        out_dir = "./%s_quant"%args.model_name
        ...
        print(dqconfig)
        quantizer = ORTQuantizer.from_pretrained("./%s_quant"%args.model_name + "/onnx", file_name="model.onnx")
        ...
        ort_sess = ort.InferenceSession('./%s_smoothquant/onnx/model.onnx'%args.model_name, providers=["CPUExecutionProvider"])
        start = time.time()
        ...
        ort_sess = ort.InferenceSession('./%s_ortquantized/model_quantized.onnx'%args.model_name, providers=["CPUExecutionProvider"])
        start = time.time()
